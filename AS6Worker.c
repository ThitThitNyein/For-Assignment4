...
void registration();
void lobby();
void gmailChecking(char gGmail[50], int userIndex);
...
void registration() {
    ...
    printf("This is Registration form:\n");
...
void gmailChecking(char gGmail[50], int userIndex) {
    ...
    int gCount = charCounting(gGmail);
    int info_gCount = charCounting(info[userIndex].email);
    int sameCount = 0;

    if (gCount == info_gCount) {
        for (int x = 0; x < gCount; x++) {
            if (gGmail[x] == info[userIndex].email[x]) {
                sameCount++;
        ...
        if (sameCount == gCount) {
            for (int y = 0; y < sameCount; y++) {

                char ggmail = gGmail[y];
                if (ggmail == '@') {
                    pFound = 1;
